...
Mesh::Mesh(vector<Vertex> vertices, vector<Vertex> cornerVertices,  int xmax, int ymax, int xmin, int ymin)
{
	this->xmax = xmax;
	this->xmin = xmin;
	this->ymax = ymax;
...
void Mesh::MakeObjFile(string name)
{
	...
	{
		objFile << "v " << cornerVertices[v].x << " " << cornerVertices[v].z << " " << cornerVertices[v].y << " " << "\n";
		offset++;
	}
	for (int v = 0; v < vertices.size(); v++)
	{
		objFile << "v " << vertices[v].x << " " << vertices[v].z << " " << vertices[v].y << " " << "\n";
	}
	...
	for (int p = 0; p < polygons.size(); p++)
	{
		if (alternate == 1) 
		{
			objFile << "f " << polygons[p].indexA + offset << " " << polygons[p].indexB + offset << " " << polygons[p].indexC + offset << " " << "\n";
			alternate = 0;
		}
		else if (alternate == 0)
		{
			objFile << "f " << polygons[p].indexC + offset << " " << polygons[p].indexB + offset << " " << polygons[p].indexA + offset << " " << "\n";
			alternate = 1;
		}
...
int Mesh::ComputeCentroidClosestToBorder()
{
	...
	for (int p = 1; p < polygons.size(); p++) 
	{
	 ...
	 return minDistanceToBorder_Y < minDistanceToBorder_X ? vertexWithShortestDistance_Index_Y : vertexWithShortestDistance_Index_X;
...
void Mesh::FindTwoClosestVertices(int index)
{
	...
	for (int v = 0; v <= 2; v++)
	{
		min_index = min_element(distances.begin()+v, distances.end()) - distances.begin();
...
vector<double> Mesh::ComputeVertexDistancesTo(int vertex_Index)
{
	...
	for (int v = 0; v < vertices.size(); v++)
	{
		if (v != vertex_Index)
		{
			x = vertices[vertex_Index].x - vertices[v].x; //calculating number to square in next step
...
void Mesh::ComputeCentroids()
{
	for (int p = 0; p < polygons.size(); p++)
	{
		polygons[p].centroid = { (vertices[polygons[p].indexA].x + vertices[polygons[p].indexB].x + vertices[polygons[p].indexC].x) / 3,(vertices[polygons[p].indexA].y + vertices[polygons[p].indexB].y + vertices[polygons[p].indexC].y) / 3, 0.0 };
	}
}
...
vector<double> Mesh::ComputeCentroidDistances(int centroid_Index, vector<Polygon> polygonClone)
{
	...
	{
		x = polygons[centroid_Index].centroid.x - polygonClone[p].centroid.x; //calculating number to square in next step
