...
class Player {
 ...
 public:
  Player(const char* name, int lives = 100)
      : _name(name), _score(0), _lives(lives) {}
...
class GameBase {
 ...
 public:
  enum State { Init, Running, Paused, Ended };
  ...
  std::string GetStateString() {
    std::string statestr = "";
    switch (state) {
      case GameBase::State::Init:
        statestr = "Init";
        break;
      case GameBase::State::Paused:
        statestr = "Paused";
        break;
      case GameBase::State::Running:
        statestr = "Running";
        break;
      case GameBase::State::Ended:
        statestr = "Ended";
        break;
    }
    std::ostringstream ss;
  ...
  virtual void Start() { state = GameBase::State::Running; }
  virtual void Pause() { state = GameBase::State::Paused; }
  virtual void Resume() { state = GameBase::State::Running; }
  virtual void End() { state = GameBase::State::Ended; }
};
