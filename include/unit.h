...
class Unit : public Entity {
  ...
  bool _mark_removed =
      false;  // todo: not needed if unit truly removed upon death
  Vec3d _lookat;
  Vec3d _valhalla;
 ...
 public:
  ...
  Unit(int id, const std::string& name, const std::string& meshname,
       AbsolutePath* pf)
      : Entity(id, name, meshname), _pf(pf) {
    _valhalla = Vec3d(500.0, 500.0, 500.0);
  }
  ...
  bool IsInValhalla() { return _pos == _valhalla ? true : false; }
